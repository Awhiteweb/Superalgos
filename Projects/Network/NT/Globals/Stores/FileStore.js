...
exports.newNetworkGlobalsStoresFileStore = function newNetworkGlobalsStoresFileStore(filename) {
    ...
    async function readFile() {
        return new Promise((res, rej) => SA.nodeModules.fs.readFile(filename, (err, data) => {
            if(err) {
                rej(err)
                return
            }
    ...
    async function writeFile(data) {
        return new Promise((res,rej) => SA.nodeModules.fs.writeFile(filename, JSON.stringify(data), (err) => {
            if(err) {
                rej(err)
                return
            }
